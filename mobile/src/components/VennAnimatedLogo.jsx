...
export default function VennAnimatedLogo({ size = 200 }) {
  ...
  const spinLeftTransform = spinLeft.interpolate({
    inputRange: [-180, 0],
    outputRange: ['rotate(-180, 82, 128)', 'rotate(0, 82, 128)'],
  });
  const spinRightTransform = spinRight.interpolate({
    inputRange: [-180, 0],
    outputRange: ['rotate(-180, 174, 128)', 'rotate(0, 174, 128)'],
  });
  ...
  useEffect(() => {
    Animated.sequence([
      ...
      // Phase 2: spin both circles into correct orientation
      Animated.parallel([
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 256 256">
      ...
      {/* Left circle: slides in from left, already at -180deg, then spins to 0 */}
      <AnimatedG transform={slideLeftTransform} opacity={slideOpacity}>
        <AnimatedG transform={spinLeftTransform}>
          <Path fill="url(#vl1)" d={LEFT_PATH} />
        </AnimatedG>
      </AnimatedG>

      {/* Right circle: slides in from right, already at -180deg, then spins to 0 */}
      <AnimatedG transform={slideRightTransform} opacity={slideOpacity}>
        <AnimatedG transform={spinRightTransform}>
